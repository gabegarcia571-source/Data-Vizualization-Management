---
title: "Tidy Tuesday Assignment Code"
...
```{r Historically Weak Passport Gains attempt 1}
...
```{r Historically Weak Passport Gains attempt 2}
...
```{r Historically Weak Passport Gains attempt 3}
...
```{r Historically Weak Passport Gains attempt Final }
...
```{r Historically Weak Passport Gains Final Plot}
...
```{r Time Series of Leading Powerhouse Passports attempt 1}
...
```{r Time Series of Leading Powerhouse Passports attempt 2}
...
```{r Time Series of Leading Powerhouse Passports attempt 3}
...
```{r Time Series of Leading Powerhouse Passports with Plot}
...
```{r Time Series of Leading Powerhouse Passports with Plot SHINY}
library(shiny)
...
```{r visa power changes broader/more informative}
...
```{r rank cleaning and visuals logic line}
...
```{r assign arrows/indicators for rank change}
##Debug
leader_growth_change <- leader_growth_change %>%
  mutate(
...
```{r dt package for arrow color rendering}
library(DT)
...
```{r original leadeboard creation}
  output$leaderboard <- renderTable({
    leaderboard_data() %>%
      transmute(
        Rank = row_number(),
...
```{r DT modification for non interactive tables}
...
```{r change since 2006 iterative change}
baseline_2006 <- rank_by_year %>%
  filter(year == 2006) %>%
  select(country, visa_2006 = visa_free_count)
```

```{r change since prior year visa free coutn}
...
```{r interactive work on Power Trend Graph}
  text = paste0( "<b>", country, "</b><br>",
                 "Year: ", year, "<br>"
                 "Visa-Free Acces: ", visa_free_count)
```

```{r key events for selected countries point pop ups}
   geom_point(
      data = events_data,
      aes(
        text = paste0(
...
```{r data join (key events and events data)}
